...
- **T-ML-050** — `asset-gen` provider abstraction with multi-provider routing (Meshy primary; self-hosted open model — TripoSR / InstantMesh — fallback). Output normalized to glTF/GLB + PBR textures. Wire `packages/proto/asset_gen.proto` into codegen.

...
│   └── asset-pipeline/      # CLI: submit / inspect / reconcile 3D asset generation jobs
...
### Epic: AI 3D assets — continuous background generation

> Atmospheric vignette environments and props are generated by AI from text/image prompts, **never inline**. Generation runs as an always-on background pipeline that converges toward a desired-state asset manifest, so finished, optimized assets are already cached in R2/CDN before a player ever reaches the vignette. This is content tooling for the *world*; it does **not** touch the deterministic player Portrait. See `05_Technical_Architecture` → *Continuous background asset generation*.

- **T-ML-050** — `asset-gen` provider abstraction with multi-provider routing (Meshy primary; self-hosted Microsoft TRELLIS fallback over an internal HTTP boundary). Output normalized to glTF/GLB + PBR textures. Wire `packages/proto/asset_gen.proto` into codegen.
  - Accept: a forced failure on the primary provider routes to the fallback with no caller change; identical inputs produce the same content-address.
- **T-ML-051** — Background generation worker: consumes generation jobs from NATS JetStream, submits to the provider, polls to completion, post-processes (mesh decimation, Draco/meshopt compression, LOD generation, thumbnail), runs an automated QA + safety/brand gate, stores to R2, records metadata + provider/version in Postgres, marks the asset ready.
  - Accept: integration test submits a job (provider stubbed), drains the queue, and produces a ready, optimized GLB under the size budget with a metadata row; the job is idempotent on retry.
- **T-ML-052** — Continuous reconciler/scheduler: periodically diffs *desired* assets (from asset manifests) against *ready* assets, enqueues whatever is missing or stale, and respects a configurable per-period generation budget cap. Supports pre-warming upcoming Seasons.

...
│   └── ml-py/               # Trait scoring, Portrait gen, reflection gen, safety classify, content CMS, 3D asset gen
├── packages/
│   ├── proto/               # Protobuf definitions for gRPC + GraphQL schemas
│   ├── design-tokens/       # Colors, typography, spacing — shared by client and web
│   └── content-schema/      # JSON Schemas for Vignettes, Seasons, Trait weights, Asset manifests
├── infra/
...
│   └── asset-pipeline/      # Historical generation experiment; not Phase-G production
...
- **T-ML-002** — Protobuf-generated gRPC server for `TraitScoringService`, `ReflectionGenService`, `PortraitGenService`. Service stubs return `Unimplemented`. (`AssetGenService` is defined in `packages/proto/asset_gen.proto` as a forward contract; wired into codegen with T-ML-050.)
...
- **T-CONTENT-004** — JSON Schema for `AssetManifest`: declares the 3D assets a Season/Vignette needs (prompt, reference images, generation params, provider hints, license, and a stable content-address). Published in `packages/content-schema/`; validated by `tools/content-validator` (`make validate-assets`).
  - Accept: validator passes on a sample asset manifest; the content-address (a stable hash of all generation inputs) is computed deterministically and is identical across runs.
...
### Epic: Historical 3D generation experiment (superseded)

> `T-ML-050` through `T-ML-052`, `T-CLIENT-040`, and `T-INFRA-040` produced useful
> schemas and fallback behavior, but the Meshy/TRELLIS + Thermion production direction
> is retired. Do not extend or activate paid generation during Phase G. The active
> Unity/Blender/CC0/Mixamo work is specified by `T-CLIENT-200`,
> `T-CLIENT-201`, and `T-ASSET-200`.
- **T-CLIENT-041** — Historical atmospheric Flutter backdrop and the current 2D fallback reference: load `VignetteBackdrop` manifests, render layered parallax + continuous ambient motion behind the choice UI, and honor the OS "reduce motion" preference. Unity must preserve its missing-scene and reduce-motion behavior.
...
- **T-INFRA-040** — Retired with the paid-generation experiment; do not provision provider keys or generation budgets for Phase G.
...
> ✅ **Direction revised and approved** (founder instruction, 2026-07-26). Echo is
> 3D-game-first. Unity 6 Personal in `apps/unity-client` is the authoritative game
> client. Blender, verified CC0 libraries, and optional Mixamo animation form the
> author-time free asset pipeline. Flutter/Thermion remains a migration reference and
> fallback only. Paid generation is retired from Phase G.
>
> ⚠️ Adding the Unity client/dependencies and changing the scene QA/brand gate require
> human review under escalation rules #1 and #12. Scene-level youth-safe review remains
> mandatory before shipping (#7/#12).

- **T-DESIGN-200** — Vision/design definition (gate: founder sign-off, blocks the rest).
  **Decision made:** grounded cinematic realism in bounded walk-around
  micro-scenes. Each scene has one immediate objective, contextual interactions,
  optional noticing, an in-world choice, a resolution, and a no-penalty
  skip-to-choice path. See `14_Graphics_Gameplay_Production_Plan`.
...
- **T-CLIENT-200** — Playable Unity vignette runtime: bounded third-person
  movement, collision-aware follow camera, contextual interactions, objective
  presentation, pause/restart, no-penalty skip, choice and deterministic ending.
  Gameplay state remains independent of rendering and missing optional art has
  a deliberate fallback.
  - Accept: a first-time player can finish the representative Bedroom vignette;
    automated play-mode coverage proves the required path and ending; movement,
    camera, reduce-motion, gamepad, and performance pass external QA.
- **T-CLIENT-201** — Unity target parity: build and verify the same scene and choice
  loop on macOS, Windows, iOS, Android, and WebGL, with still/2D degradation where the
  target cannot meet its performance budget.
  - Accept: every target renders the composed scene or a deliberate accessible fallback;
    offline play uses bundled/cached content.
- **T-CONTENT-200** — Scene composition schema + authoring: extend `VignetteBackdrop`
  ...
  lighting, and camera bounds. Prove bedroom, street/bus-stop, and café/shop first;
  author all Season-1 scenes only after that art-direction gate passes.
  - Accept: schema in `packages/content-schema/`; the three proof scenes pass external
    review; later all 20 vignettes have a scene; `make replay` remains unchanged.
- **T-ASSET-200** — Reproducible free asset pipeline: validate/optimize FBX or GLB in
  Blender, record provenance and licensing, import into Unity, and enforce per-scene
  triangle/material/texture budgets. Mixamo files stay local unless redistribution is
  explicitly permitted, with a checked-in fallback.
  - Accept: `make unity-validate-mixamo` validates the optional local character; a clean
    checkout runs without it; every shipped asset has provenance and performance data.
...
Verification additions for this milestone: `make validate-scenes`, `make
unity-validate-mixamo` when the local optional FBX exists,
`make unity-validate-bedroom-assets`, `make unity-test`, and
`make unity-build-macos` (plus target-specific builds in CI).
...
11. **Triggering paid external generation APIs at scale** (batch or continuous 3D/asset generation runs) or **changing generation budget caps** — anything that materially affects spend.
